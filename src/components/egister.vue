					...
					<div class="main-register">
						...
						<div class="register-input" v-show="isPhones">
							...
							<div class="select-box positionR">
                               ...
                               <input type="text" class="input-main input-content phone-account" v-model="phone"  maxlength="11" style="padding-left: 122px;">
							</div>
						...
						<div class="register-input" v-show="isEmail">
							...
							<input type="text" class="input-main input-content email-account" v-model="phone">
						</div>
						...
						<div class="register-input mbcode-box" v-if="!isEmail">
							...
							<div class="msg-code tc baseColor curPer">
								<span class="mbcode-btn">获取验证码</span>
							</div>
						</div>
						<div class="register-input emcode-box" v-if="isEmail">
							<span class="register-item">邮箱验证码</span>
							<input type="password" class="input-main input-content" maxlength="6" v-model="emcode">
							<div class="msg-code tc baseColor curPer">
								<span class="emcode-btn">获取验证码</span>
							</div>
						</div>
						<div class="register-input">
							<span class="register-item">登录密码</span>
							<input type="password"  class="input-main input-content pwd-input" v-model="thepwd" >
						</div>
						<div class="register-input">
							<span class="register-item">确认密码</span>
							<input type="password" class="input-main input-content repwd-input" v-model="therepwd" >
						</div>
						<div class="register-input">
						...
						<div style="margin-top: 10px;">
							...
							<button class="register-button" @click="confirmReg">注册</button>
	...
	export default {
		...
		methods:{
			confirmReg(){
				...
				console.log(this.isEmail,this.isPhones);
				var data = {};
				if(this.phone == ''){
					if(this.isEmail){
						layer.tips('请输入账号','.email-account');return;
					} else {
						layer.tips('请输入账号','.phone-account');return;
					}
				} else if(this.isEmail){
					if(this.emcode == ''){
						layer.tips('请输入验证码','.emcode-btn');return;
					}
				} else if(this.isPhones){
					
					if(this.mbcode == ''){
					alert('cnm');
						alert('haha')
						layer.tips('请输入验证码','.mbcode-btn');return;
					}
				} else if(this.thepwd == ''){
						layer.msg('请输入密码');return;

				}else if(this.therepwd == ''){
				...
				data.user_string = this.phone;
				data.password = this.pwd;
				data.re_password = this.re_pwd;
				data.type = this.isEmail?'email':'mobile';
				data.code = this.isEmail?this.emcode:this.mbcode;
				data.extension_code = this.parent;
				this.$http({
